...
function Settings() {
  ...
  return (
    <>
      ...
      <h1>Settings</h1>
      <img src={`data:image/png;base64,${allyLogo}`} alt="example bank logo" className="h-8 w-8"/>
      <img src={`data:image/png;base64,${nfcuLogo}`} alt="example bank logo" className="h-8 w-8"/>
      <button onClick={() => open()} disabled={!ready}>Add Bank</button>
    </>
  )
}
